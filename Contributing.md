...
Add an entry for your tool in `data/tools.json` (inside the `"tools"` array). Follow the existing entries as a guide — you'll need these fields:
...
### Step 7 — Sort and normalize `data/tools.json`

Run the sort-norm script to alphabetically sort tools, normalize tags, and sort tech stacks in `data/tools.json`:
...
> **Note:** `scripts/build.js` reads `scripts/index-template.txt` and injects tool data from `data/tools.json` to generate `index.html`. You do **not** need to edit the template, the landing page, or the Available Tools table in `ReadMe.md` — those are auto-generated by the build scripts and `node scripts/sync-readme.js`, which the maintainer runs after merging your PR.
...
Beyond tools, you can contribute **resume** and **portfolio** themes. Each theme is a single [Handlebars](https://handlebarsjs.com/) template (`.hbs`) that gets compiled against `data/profile.json` to produce a standalone HTML file.

### How it works

1. `data/profile.json` contains sample profile data (personal info, experience, skills, projects, etc.)
2. `scripts/theme-gen.js` discovers all `.hbs` files in `resume/` and `portfolio/`
...
Add an entry for your theme in `data/themes.json` under the `"resume"` or `"portfolio"` array:
...
This is how your theme appears on the landing page — similar to how `data/tools.json` works for tools. Everything is derived from the `id`:
...
node scripts/theme-gen.js             # Generate HTML from all .hbs templates
